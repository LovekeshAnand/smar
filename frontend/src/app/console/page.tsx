...
export default function Home() {
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      const asstMsg: ChatMessage = {
        ...
        dbContext: data.db_context || data.operation_details?.db_context || data.smart_data?.db_context || null,
      };
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-[#09090b] text-zinc-100 overflow-hidden font-sans select-none relative">
      {/* Dynamic Gradient Waves WebGL (Vibrant, Identical to Landing Page) */}
      ...
      <main className="flex-1 w-full h-[calc(100vh-64px)] overflow-hidden relative z-10 px-3 sm:px-6 pb-4 pt-2">
        <div
        ...
        >
          ...
          <div
            className={`transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col items-center ${
              hasStarted
                ? "w-full lg:w-[380px] xl:w-[410px] shrink-0 h-full bg-zinc-950/75 backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-5 shadow-2xl justify-between overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-left-6"
            ...
            {!hasStarted && (
              <div className="w-full flex flex-col items-center text-center mt-2 space-y-6 animate-in fade-in duration-500">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-zinc-400 font-normal mt-1.5 max-w-md">
          ...
          {/* ========================================================================= */}
          <div
            className={`transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col bg-zinc-950/75 backdrop-blur-2xl border border-white/[0.08] rounded-3xl shadow-2xl overflow-hidden ${
              hasStarted
              ...
              <>
                ...
                <div className="flex items-center justify-between pb-3 mb-2 border-b border-white/[0.08] shrink-0">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h2 className="text-xs font-mono font-semibold tracking-wide text-zinc-200 uppercase">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-mono text-zinc-400 hover:text-white bg-zinc-900/60 hover:bg-zinc-800 border border-zinc-800 px-2.5 py-1 rounded-xl transition-all cursor-pointer"
                ...
                <form onSubmit={handleRightSubmit} className="mt-2 shrink-0">
                  <div className="flex items-center gap-2 bg-zinc-900/80 border border-white/[0.1] focus-within:border-white/30 rounded-2xl px-3 py-2 transition-all shadow-inner">
                    <button
                      type="button"
                      onClick={toggleMicrophone}
                      className={`p-2 rounded-xl transition-all cursor-pointer ${
                        isRecording
                          ? "bg-white text-zinc-950 animate-pulse"
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent border-none outline-none text-zinc-100 placeholder:text-zinc-500 text-xs sm:text-sm font-sans"
                    />

                    <button
                      type="submit"
                      disabled={!rightInputText.trim()}
                      className="px-4 py-2 rounded-xl bg-white hover:bg-zinc-200 text-zinc-950 font-semibold text-xs tracking-wide transition-all disabled:opacity-30 disabled:pointer-events-none flex items-center gap-1.5 shadow-sm cursor-pointer"
                    >
                      <span>Send</span>
